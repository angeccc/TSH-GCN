...
h36m_skeleton = Skeleton(parents=[-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 12, 16, 17, 18, 19, 20, 19, 22, 12, 24, 25, 26, 27, 28, 27, 30],
                         joints_left=[6, 7, 8, 9, 10, 16, 17, 18, 19, 20, 21, 22, 23], joints_right=[1, 2, 3, 4, 5, 24, 25, 26, 27, 28, 29, 30, 31])

h36m_cameras_intrinsic_params = [
    {'id': '54138969', 'center': [512.54150390625, 515.4514770507812], 'focal_length': [1145.0494384765625, 1143.7811279296875],
     'radial_distortion': [-0.20709891617298126, 0.24777518212795258, -0.0030751503072679043], 'tangential_distortion': [-0.0009756988729350269, -0.00142447161488235],  #切向畸变
     'res_w': 1000, 'res_h': 1002, 'azimuth': 70, },

    {'id': '55011271', 'center': [508.8486328125, 508.0649108886719], 'focal_length': [1149.6756591796875, 1147.5916748046875],
     'radial_distortion': [-0.1942136287689209, 0.2404085397720337, 0.006819975562393665], 'tangential_distortion': [-0.0016190266469493508, -0.0027408944442868233],
     'res_w': 1000, 'res_h': 1000, 'azimuth': -70, },

    {'id': '58860488', 'center': [519.8158569335938, 501.40264892578125], 'focal_length': [1149.1407470703125, 1148.7989501953125],
     'radial_distortion': [-0.2083381861448288, 0.25548800826072693, -0.0024604974314570427], 'tangential_distortion': [0.0014843869721516967, -0.0007599993259645998],
     'res_w': 1000, 'res_h': 1000, 'azimuth': 110, },

    {'id': '60457274', 'center': [514.9682006835938, 501.88201904296875], 'focal_length': [1145.5113525390625, 1144.77392578125],
     'radial_distortion': [-0.198384091258049, 0.21832367777824402, -0.008947807364165783], 'tangential_distortion': [-0.0005872055771760643, -0.0018133620033040643],
     'res_w': 1000, 'res_h': 1002, 'azimuth': -110, },
]
h36m_cameras_extrinsic_param = {
    'S1': [{'orientation': [0.1407056450843811, -0.1500701755285263, -0.755240797996521, 0.6223280429840088],
    ...
    'S5': [{'orientation': [0.1467377245426178, -0.162370964884758, -0.7551892995834351, 0.6178938746452332],
    ...
    'S6': [{'orientation': [0.1337897777557373, -0.15692396461963654, -0.7571090459823608, 0.6198879480361938],
    ...
    'S7': [{'orientation': [0.1435241848230362, -0.1631336808204651, -0.7548328638076782, 0.6188824772834778],
    ...
    'S8': [{'orientation': [0.14110587537288666, -0.15589867532253265, -0.7561917304992676, 0.619644045829773],
    ...
    'S9': [{'orientation': [0.15540587902069092, -0.15548215806484222, -0.7532095313072205, 0.6199594736099243],
    ...
    'S11': [{'orientation': [0.15232472121715546, -0.15442320704460144, -0.7547563314437866, 0.6191070079803467],
...
class Human36mDataset(MocapDataset):
    ...
    #storage camera paras
        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_param)
        self.in_cameras = copy.deepcopy(h36m_cameras_intrinsic_params)

        for cameras in self._cameras.values():
        #
            for i, cam in enumerate(cameras):     #0-3， {ori,tra}  enumerate(list)
                cam.update(self.in_cameras[i])  #
                for k, v in cam.items():           # dict.items()
                    if k not in ['id', 'res_w', 'res_h']:
                        cam[k] = np.array(v, dtype='float32')   #

                cam['intrinsic_matrix'] = np.array([[cam['focal_length'][0], 0, cam['center'][0]], #dict['']
                ...
                if 'translation' in cam:
                    cam['translation'] = cam['translation'] / 1000  # mm to meter
    ...
    #load 3d-gt
        ...
        self._data = {}
        for subject, actions in data.items():  #'S1', {'sitting':ndarray(3304,32,3), 'act2': ......}
            self._data[subject] = {}
            for action_name, positions in actions.items():  #'Sitting 1',  ndarray(3304,32,3)
                self._data[subject][action_name] = { 'positions': positions,   'cameras': self._cameras[subject] }

        if remove_static_joints: #
            self.remove_joints([4, 5, 9, 10, 11, 16, 20, 21, 22, 23, 24, 28, 29, 30, 31]) #self.data[subject][action_name]
